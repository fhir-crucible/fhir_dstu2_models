...
class ExpansionsTest < Test::Unit::TestCase
  def test_expansion
    codes = FHIR::DSTU2::Definitions.get_codes('http://hl7.org/fhir/ValueSet/relatedperson-relationshiptype')
    assert (!codes.nil? && !codes.empty?), 'Expansions did not return expected codes.'
  end

  def test_missing_expansion
    codes = FHIR::DSTU2::Definitions.get_codes('http://projectcrucible.org/nonexisting/valueset')
