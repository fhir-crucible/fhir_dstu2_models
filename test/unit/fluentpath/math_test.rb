...
class MathTest < Test::Unit::TestCase
  ...
  def test_plus_vars
    result = FluentPath::DSTU2.evaluate('x + y', DATA)
    assert result == 10, 'Failed addition test.'
  end

  def test_plus_nums
    result = FluentPath::DSTU2.evaluate('5 + 5', DATA)
    assert result == 10, 'Failed addition test.'
  end

  def test_plus_vars_strings
    result = FluentPath::DSTU2.evaluate("foo + bar", DATA)
    assert result == 'ab', 'Failed addition test.'
  end

  def test_plus_strings
    result = FluentPath::DSTU2.evaluate("'Joe' + ' ' + 'Smith'", DATA)
  ...
  def test_minus_nums
    result = FluentPath::DSTU2.evaluate('5 - 1', DATA)
    assert result == 4, 'Failed subtraction test.'
  end
  ...
  def test_divide_nums
    result = FluentPath::DSTU2.evaluate('6 / 3', DATA)
    assert result == 2, 'Failed division test.'
  end
