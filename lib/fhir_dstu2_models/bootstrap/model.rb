  ...
  module DSTU2
    class Model
      ...
      def validate_field(field, value, contained_here, meta, errors)
        ...
        datatype = meta['type']
        value.each do |v|
          klassname = v.class.name.gsub('FHIR::DSTU2::', '')
          # if the data type is a generic Resource, validate it
          if datatype == 'Resource'
            if FHIR::DSTU2::RESOURCES.include?(klassname)
          ...
          # if the data type is a primitive, test the regular expression (if any)
          elsif FHIR::DSTU2::PRIMITIVES.include?(datatype)
            primitive_meta = FHIR::DSTU2::PRIMITIVES[datatype]
            if primitive_meta['regex'] && primitive_meta['type'] != 'number'
              ...
              errors[field] << "#{meta['path']}: #{v} is not a valid #{datatype}" unless FHIR::DSTU2.primitive?(datatype: datatype, value: v)
            end
          end
          # check binding
      ...
      def validate_reference_type(ref, meta, contained_here, errors)
        ...
        meta['type_profiles'].each do |p|
          ...
          profile_basetype = FHIR::DSTU2::Definitions.basetype(p)
        ...
        if !matches_one_profile && ref.reference.start_with?('#')
          ...
          if !r.nil?
            ...
            FHIR::DSTU2.logger.warn "Unable to resolve reference #{ref.reference}"
          end
        end
