    ...
    module Boot
      class Template
        ...
        def to_s(offset = 2)
          ...
          # always declare the FHIR::DSTU2 module
          s << "module FHIR\n  module DSTU2" if @top_level
          ...
          @name.each_with_index do |name, index|
            ...
            classdef += ' < FHIR::DSTU2::Model' if type == 'class'
            s << classdef
          end

          # include modules
          space = indent(@name.length + 1, offset)
          s << "#{space}include FHIR::DSTU2::Hashable" unless @name.empty?
          s << "#{space}include FHIR::DSTU2::Json" unless @name.empty?
          s << "#{space}include FHIR::DSTU2::Xml" unless @name.empty?
          s << ''
