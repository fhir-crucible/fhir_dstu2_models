module FluentPath
  module DSTU2
    ...
    def self.evaluate(expression, hash, parent = nil)
      @@context = hash
      @@parent = parent
      tree = FluentPath::DSTU2.parse(expression)
      FHIR::DSTU2.logger.debug "TREE: #{tree}"
      compute(tree, hash)
    end
    ...
    def self.compute(tree, data)
      tree = tree.tree if tree.is_a?(FluentPath::DSTU2::Expression)
      # --------------- OPERATOR PRECEDENCE ------------------
      ...
      while tree.length != size || substitutions > 0
        substitutions = 0
        FHIR::DSTU2.logger.debug "DATA: #{tree}"
        previous_node = nil
        previous_index = nil
        size = tree.length
        tree.each_with_index do |node, index|
          if node.is_a?(String) && !(node.start_with?("'") && node.end_with?("'"))
            ...
            elsif !previous_node.is_a?(FluentPath::DSTU2::Expression)
              tree[index] = get(node, data)
            end
            if array_index && tree[index].is_a?(Array)
              tree[index] = tree[index][array_index]
            end
            FHIR::DSTU2.logger.debug "V===> #{tree}"
          elsif node.is_a?(Symbol) && functions.include?(node)
            previous_node = compute(previous_node, data) if previous_node.is_a?(FluentPath::DSTU2::Expression)
            case node
            when :where
              # the previous node should be data (as Array or Hash)
              # the next node should be a block or subexpression (as FluentPath::DSTU2::Expression)
              block = tree[index + 1]
              if block.is_a?(FluentPath::DSTU2::Expression)
                tree[index + 1] = nil
              else
            ...
            when :extension
              ...
              # the next node optionally is a block or subexpression (as FluentPath::DSTU2::Expression)
              block = tree[index + 1]
              if block.is_a?(FluentPath::DSTU2::Expression)
                tree[index + 1] = nil
              else
              ...
              if previous_node.is_a?(Hash)
                FHIR::DSTU2.logger.debug 'Evaling Extension Block....'
                exts = data['extension']
                if exts.is_a?(Array)
                  url = nil
                  begin
            ...
            FHIR::DSTU2.logger.debug "F===> #{tree}"
          end
          previous_index = index
          previous_node = tree[index]
        end
        FHIR::DSTU2.logger.debug '---------------------------------------------------'
        tree.compact!
      end
      tree.each_with_index do |node, index|
      ...
      while tree.length != size
        FHIR::DSTU2.logger.debug "FUNC: #{tree}"
        previous_node = data
        previous_index = nil
        size = tree.length
        tree.each_with_index do |node, index|
          if node.is_a?(Symbol) && functions.include?(node)
            ...
            when :startsWith
              ...
              if previous_node.is_a?(String)
                FHIR::DSTU2.logger.debug 'Evaling StartsWith Block....'
            ...
            when :substring
              ...
              if previous_node.is_a?(String)
                ...
                if args.is_a?(String) && args.include?(',')
                  ...
                  FHIR::DSTU2.logger.debug 'Evaling Substring Block....'
            ...
            when :contains
              ...
              if previous_node.is_a?(String)
                FHIR::DSTU2.logger.debug 'Evaling Contains Block....'
            ...
            when :in
              ...
              # the next node should an Array (possibly as a block or subexpression/FluentPath::DSTU2::Expression)
              block = tree[index + 1]
              if block.is_a?(FluentPath::DSTU2::Expression)
                FHIR::DSTU2.logger.debug 'Evaling In Block....'
      ...
      size = -1
      while tree.length != size
        FHIR::DSTU2.logger.debug "MATH: #{tree}"
        previous_node = nil
        previous_index = nil
        size = tree.length
        tree.each_with_index do |node, index|
          if node.is_a?(Symbol) && functions.include?(node)
            previous_node = compute(previous_node, data) if previous_node.is_a?(FluentPath::DSTU2::Expression)
            tree[index + 1] = compute(tree[index + 1], data) if tree[index + 1].is_a?(FluentPath::DSTU2::Expression)
            left = previous_node
            right = tree[index + 1]
            case node
            when :"/"
      ...
      size = -1
      while tree.length != size
        FHIR::DSTU2.logger.debug "EQ: #{tree}"
        previous_node = nil
        previous_index = nil
        size = tree.length
        tree.each_with_index do |node, index|
      ...
      size = -1
      while tree.length != size
        FHIR::DSTU2.logger.debug "LOGIC: #{tree}"
        previous_node = nil
        previous_index = nil
        size = tree.length
        tree.each_with_index do |node, index|
      ...
      size = -1
      while tree.length != size
        FHIR::DSTU2.logger.debug "IMPLIES: #{tree}"
        previous_node = nil
        previous_index = nil
        size = tree.length
        tree.each_with_index do |node, index|
      ...
      FHIR::DSTU2.logger.debug "OUT: #{tree}"

      tree.map! do |out|
        while out.is_a?(FluentPath::DSTU2::Expression)
          out = compute(out, data)
        end
        out
      end

      FHIR::DSTU2.logger.debug "RETURN: #{tree.first}"
      tree.first
    end
  end
end